...
export default () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post('https://ab.execute-api.ap-south-1.amazonaws.com/production/api/post/work', pageData, {
        headers: {
          Authorization: `${token}`
        }
      });
  ...
  useEffect(() => {
    axios.get(`https://ab.execute-api.ap-south-1.amazonaws.com/production/api/get/work?page=${currentPage}&perPage=${itemsPerPage}`)
      .then(response => {
  ...
  const handleDelete = (id) => {
    const token = localStorage.getItem('token');
    axios.delete(`https://ab.execute-api.ap-south-1.amazonaws.com/production/api/delete/work/${id}`, {
      headers: {
        Authorization: `${token}`
      }
    })
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      const response = await axios.put(`https://ab.execute-api.ap-south-1.amazonaws.com/production/api/update/work/${editItemId}`, editData, {
        headers: {
          Authorization: `${token}`
        }
      });
