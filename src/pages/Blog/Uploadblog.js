...
export default () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post('https://ab.execute-api.ap-south-1.amazonaws.com/production/api/createblog', pageData, {
                headers: {
                    Authorization: `${token}`
                }
            });
    ...
    const handleDelete = (id) => {
        const token = localStorage.getItem('token');
        axios.delete(`https://ab.execute-api.ap-south-1.amazonaws.com/production/api/blog/${id}`, {
            headers: {
                Authorization: `${token}`
            }
        })
    ...
    const handleEditSubmit = async () => {
        ...
        try {
            const response = await axios.put(`https://ab.execute-api.ap-south-1.amazonaws.com/production/api/blog/${editItemId}`, editData, {
                headers: {
                    Authorization: `${token}`
                }
            });
